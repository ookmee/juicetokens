...
import { SignatureUtil, HashUtil } from './utils/crypto';
import {
  IValidationError,
  ValidationErrorCode,
  IResult
...
import {
  ...
  IStageMetric,
  IStageTransition,
  IUserDevelopmentalStageStatus,
  IDevelopmentalStageActivity,
  IActivityCompletion,
  IStageProgressionRecommendation,
  IDevelopmentalStageChallenge,
  IStageMilestone
 ...
 */
export class DevelopmentalStageAdapter {
  private readonly MIN_PROGRESS_FOR_TRANSITION = 0.8; // Minimum progress required before transitioning
  private readonly MIN_MASTERY_FOR_TRANSITION = 0.7; // Minimum mastery required before transitioning
  private readonly SECONDARY_STAGE_CONTRIBUTION = 0.3; // 30% contribution from secondary stage activities
  private readonly PRIMARY_STAGE_CONTRIBUTION = 0.7; // 70% contribution from primary stage activities
  private readonly BASE_EXPERIENCE_PER_LEVEL = 1000; // Base experience points required per level
   ...
   */
  public initializeUserStage(userId: string): IResult<IUserDevelopmentalStageStatus> {
    if (!userId) {
      return {
        success: false,
        errors: [{
    ...
    const now = Date.now();
    const trustStageMetric: IStageMetric = {
    ...
    const stageMetrics = new Map<string, IStageMetric>();
    stageMetrics.set(DevelopmentalStageLevel.TRUST.toString(), trustStageMetric);
    
    const userStatus: IUserDevelopmentalStageStatus = {
      userId,
      stageMetrics,
      currentPrimaryStage: DevelopmentalStageLevel.TRUST,
      currentSecondaryStage: DevelopmentalStageLevel.TRUST,
   ...
   */
  public recordActivityCompletion(
    userStatus: IUserDevelopmentalStageStatus,
    activity: IDevelopmentalStageActivity,
    evidence?: Uint8Array,
    validatorIds: string[] = []
  ): IResult<{userStatus: IUserDevelopmentalStageStatus, activityCompletion: IActivityCompletion}> {
    ...
    const completion: IActivityCompletion = {
      completionId: uuidv4(),
    ...
    if (currentPrimaryMetric && 
      ...
      if (userStatus.currentPrimaryStage < DevelopmentalStageLevel.GENERATIVITY) {
  ...
  public transitionStage(
    userStatus: IUserDevelopmentalStageStatus,
    newStage: DevelopmentalStageLevel,
    transitionTrigger: string = 'natural_progression',
    witnessIds: string[] = []
  ): IResult<IUserDevelopmentalStageStatus> {
    const errors: IValidationError[] = [];
    ...
    if (newStage === undefined || newStage < 0 || newStage > DevelopmentalStageLevel.GENERATIVITY) {
      errors.push({
        field: 'newStage',
    ...
    // Update user status
    const updatedStatus: IUserDevelopmentalStageStatus = {
      ...userStatus,
   ...
   */
  public generateProgressionRecommendations(
    userStatus: IUserDevelopmentalStageStatus,
    availableActivities: IDevelopmentalStageActivity[],
    userConnections: string[] = []
  ): IResult<IStageProgressionRecommendation> {
    const errors: IValidationError[] = [];
    ...
    const relevantActivities = availableActivities.filter(
      activity => activity.primaryStage === currentStage || 
    ...
    relevantActivities.sort((a, b) => {
      if (a.primaryStage === currentStage && b.primaryStage !== currentStage) return -1;
      if (a.primaryStage !== currentStage && b.primaryStage === currentStage) return 1;
    ...
    const recommendation: IStageProgressionRecommendation = {
      userId: userStatus.userId,
      currentStage,
      currentProgress,
  ...
  public recordMilestoneCompletion(
    ...
    if (milestone.isStageTransition && 
        ...
        userStatus.currentPrimaryStage < DevelopmentalStageLevel.GENERATIVITY) {
      
      const nextStage = userStatus.currentPrimaryStage + 1;
      const transitionResult = this.transitionStage(
        userStatus, 
        nextStage, 
        `milestone_completion:${milestone.milestoneId}`
      );
  ...
  public validateChallenge(
    userStatus: IUserDevelopmentalStageStatus, 
    challenge: IDevelopmentalStageChallenge
  ): IResult<boolean> {
    const errors: IValidationError[] = [];
   ...
   */
  private calculateStageProgress(stageMetric: IStageMetric): number {
   ...
   */
  private calculateMasteryScore(stageMetric: IStageMetric): number {
   ...
   */
  private generatePersonalizedGuidance(stage: DevelopmentalStageLevel, progress: number): string {
   ...
   */
  private getStageName(stage: DevelopmentalStageLevel): string {
    switch (stage) {
      case DevelopmentalStageLevel.TRUST:
        return 'Trust';
      case DevelopmentalStageLevel.AUTONOMY:
        return 'Autonomy';
      case DevelopmentalStageLevel.IMAGINATION:
        return 'Imagination';
      case DevelopmentalStageLevel.COMPETENCE:
        return 'Competence';
      case DevelopmentalStageLevel.IDENTITY:
        return 'Identity';
      case DevelopmentalStageLevel.CONNECTION:
        return 'Connection';
      case DevelopmentalStageLevel.GENERATIVITY:
        return 'Generativity';
      default:
        return 'Unknown';
    }
