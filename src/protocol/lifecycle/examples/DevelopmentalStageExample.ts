...
async function runDevelopmentalStageExample() {
  ...
  console.log(`User initialized at ${DevelopmentalStageLevel[userStatus.currentPrimaryStage]} stage`);
  ...
  // Define a milestone
  const trustMilestone: IStageMilestone = {
  ...
  const trustTransitionMilestone: IStageMilestone = {
  ...
  const recommendationsResult = stageAdapter.generateProgressionRecommendations(
    userStatus,
    activities,
    ['user-friend-1', 'user-friend-2']
  );
  ...
  if (!transitionResult.success) {
    ...
    console.log(`User has transitioned to ${DevelopmentalStageLevel[userStatus.currentPrimaryStage]} stage!`);
    printUserProgress(userStatus);
  }
  ...
  userStatus.stageHistory.forEach((transition, index) => {
    console.log(`${index + 1}. ${DevelopmentalStageLevel[transition.fromStage]} → ${DevelopmentalStageLevel[transition.toStage]}`);
 ...
 */
function printUserProgress(userStatus: any) {
  console.log(`Current stage: ${DevelopmentalStageLevel[userStatus.currentPrimaryStage]} (Primary), ${DevelopmentalStageLevel[userStatus.currentSecondaryStage]} (Secondary)`);
