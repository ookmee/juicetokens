 ...
 */
export class FilesystemDHTStorage implements DHTStorage {
  ...
  async get(key: Uint8Array): Promise<DHTEntry | null> {
    ...
    try {
      ...
      return Promise.resolve(DHTEntry.create(entryData));
    } catch (error) {
    ...
    for (const file of files) {
      if (file.endsWith('.json')) {
        ...
        try {
          ...
          const entryData = JSON.parse(fileContent);
          entries.push(DHTEntry.create(entryData));
        } catch (error) {
          console.error(`Error reading DHT entry: ${error}`);
        }
      }
