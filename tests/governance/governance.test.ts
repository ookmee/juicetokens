...
describe('Governance Layer Components', () => {
  ...
  describe('Proposal Manager', () => {
    ...
    test('should submit proposals for voting', () => {
      ...
      expect(updatedProposal!.status).toBe(ProposalStatus.SUBMITTED);
    });
  ...
  describe('Policy Enforcer', () => {
    ...
    test('should get rules for a domain', () => {
      const domainRules = policyEnforcer.getRulesForDomain(PolicyDomain.TOKEN_TRANSFER);
    ...
    test('should apply policies and allow valid actions', () => {
      ...
      const result = policyEnforcer.applyPolicies(PolicyDomain.TOKEN_TRANSFER, {
    ...
    test('should enable and disable rules', () => {
      ...
      const result2 = policyEnforcer.applyPolicies(PolicyDomain.TOKEN_TRANSFER, {
        testValue: 150
      });
    ...
    test('should maintain execution history', () => {
      ...
      policyEnforcer.applyPolicies(PolicyDomain.TOKEN_TRANSFER, {
        testValue: 75
      });
  ...
  describe('Governance Integration', () => {
    test('should integrate all governance components', () => {
      ...
      const newVersionRule: IPolicyRule = {
        ...
        action: (context) => {
          ...
          if (context.proposalStatus === ProposalStatus.ACCEPTED) {
      ...
      governance.policyEnforcer.addRule(newVersionRule, [PolicyDomain.PROTOCOL_UPGRADE]);
      
      // 2. Create a proposal for a version update
      const updateProposal = governance.proposalManager.createProposal({
      ...
      governance.proposalManager.submitProposal(updateProposal.id);
      const proposal = governance.proposalManager.getProposal(updateProposal.id)!;
      proposal.status = ProposalStatus.ACCEPTED;
      
      // 4. Try to apply the policy with the accepted proposal
      const allowUpdate = governance.policyEnforcer.applyPolicies(
      ...
      governance.versionManager.registerVersion({
        version: '0.2.0',
      ...
      governance.proposalManager.markAsImplemented(updateProposal.id);
      const implementedProposal = governance.proposalManager.getProposal(updateProposal.id);
      
      expect(implementedProposal!.status).toBe(ProposalStatus.IMPLEMENTED);
    });
  });
}); 