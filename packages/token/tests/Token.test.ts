import { Token, TokenStatus, VALID_DENOMINATIONS, TokenDenomination } from '../src/models/Token';
...
describe('Token', () => {
  ...
  describe('constructor', () => {
    it('should create a Token with the provided properties', () => {
      const props = {
        tokenId,
        denomination: 10 as TokenDenomination,
    ...
    it('should throw an error for invalid denomination', () => {
      const props = {
        tokenId,
        denomination: 3 as any, // Invalid denomination
        creationTimeMs: Date.now(),
        issuer: 'test-issuer',
        status: TokenStatus.ACTIVE
      };
      
      expect(() => new Token(props)).toThrow('Invalid denomination: 3');
    });
  });
  
  describe('create', () => {
    it('should create a new active token', () => {
      const denomination = 5 as TokenDenomination;
      const issuer = 'test-issuer';
      
      const token = Token.create(tokenId, denomination, issuer);
    ...
    it('should accept an optional expiryTimeMs', () => {
      ...
      const token = Token.create(tokenId, denomination, issuer, expiryTimeMs);
  ...
  describe('setStatus', () => {
    it('should update the token status', () => {
      const token = Token.create(tokenId, 10 as TokenDenomination, 'test-issuer');
      
      token.setStatus(TokenStatus.RESERVED);
      
      expect(token.status).toBe(TokenStatus.RESERVED);
    });
  });
  ...
  describe('isExpired', () => {
    ...
    it('should return false if current time is before expiry time', () => {
      const expiryTimeMs = Date.now() + 3600000; // 1 hour from now
      const token = Token.create(tokenId, 10 as TokenDenomination, 'test-issuer', expiryTimeMs);
  ...
  describe('validate', () => {
    ...
    it('should return false if tokenId is invalid', () => {
      ...
      const token = Token.create(invalidTokenId, 10 as TokenDenomination, 'test-issuer');
  ...
  describe('toJSON / fromJSON', () => {
    it('should correctly serialize and deserialize a Token', () => {
      const original = Token.create(tokenId, 10 as TokenDenomination, 'test-issuer');
      const json = original.toJSON();
      const deserialized = Token.fromJSON(json);
    ...
    it('should preserve expiryTimeMs during serialization', () => {
      const expiryTimeMs = Date.now() + 3600000; // 1 hour from now
      const original = Token.create(tokenId, 10 as TokenDenomination, 'test-issuer', expiryTimeMs);
      const json = original.toJSON();
      const deserialized = Token.fromJSON(json);
