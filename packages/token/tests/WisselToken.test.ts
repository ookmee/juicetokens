import { Token, TokenStatus, TokenDenomination } from '../src/models/Token';
...
describe('WisselToken', () => {
  ...
  beforeEach(() => {
    const tokenId = TokenId.generate('AMS-001', 1);
    baseToken = Token.create(tokenId, 1 as TokenDenomination, 'test-issuer');
  });
  ...
  describe('constructor', () => {
    it('should create a WisselToken with the provided properties', () => {
      ...
      const wisselToken = new WisselToken(props);
    ...
    it('should throw an error if afrondingsbuffer is negative', () => {
      ...
      expect(() => new WisselToken(props)).toThrow('Afrondingsbuffer must be between 0.0 and 0.99');
    });
  ...
  describe('create', () => {
    it('should create a new WisselToken with zero afrondingsbuffer', () => {
      const wisselToken = WisselToken.create(baseToken);
  ...
  describe('addToBuffer', () => {
    ...
    beforeEach(() => {
      wisselToken = WisselToken.create(baseToken);
    });
  ...
  describe('transferBuffer', () => {
    let sourceWisselToken: WisselToken;
    let targetWisselToken: WisselToken;
    
    beforeEach(() => {
      const sourceTokenId = TokenId.generate('AMS-001', 1);
      const sourceBaseToken = Token.create(sourceTokenId, 1 as TokenDenomination, 'test-issuer');
      sourceWisselToken = WisselToken.create(sourceBaseToken);
      sourceWisselToken.addToBuffer(0.5, 'tx-init');
      
      const targetTokenId = TokenId.generate('AMS-001', 2);
      const targetBaseToken = Token.create(targetTokenId, 1 as TokenDenomination, 'test-issuer');
      targetWisselToken = WisselToken.create(targetBaseToken);
    });
    
    it('should transfer amount between WisselTokens', () => {
      const transactionId = 'tx-transfer';
      
      const result = WisselToken.transferBuffer(
        sourceWisselToken, 
        targetWisselToken, 
        0.3, 
        transactionId
  ...
  describe('validate', () => {
    ...
    it('should return false if baseToken is invalid', () => {
      const invalidTokenId = new TokenId({
        id: '',
        issuanceId: 'AMS-001',
        sequenceNumber: 1,
      ...
      const invalidBaseToken = Token.create(invalidTokenId, 1 as TokenDenomination, 'test-issuer');
      const wisselToken = WisselToken.create(invalidBaseToken);
    ...
    it('should return false if afrondingsbuffer is invalid', () => {
      ...
      (wisselToken as any)._afrondingsbuffer = 2.0;
    ...
    it('should return false if lastUpdatedTimeMs is invalid', () => {
      ...
      // Directly manipulating private property for testing
      (wisselToken as any)._lastUpdatedTimeMs = 0;
    ...
    it('should return false if lastTransactionId is empty string', () => {
      ...
      // Directly manipulating private property for testing
      (wisselToken as any)._lastTransactionId = '';
  ...
  describe('toJSON / fromJSON', () => {
    it('should correctly serialize and deserialize a WisselToken', () => {
      ...
      const deserialized = WisselToken.fromJSON(json);
