...
describe('Telomeer', () => {
  ...
  describe('constructor', () => {
    it('should create a Telomeer with the provided properties', () => {
      ...
      const telomeer = new Telomeer(props);
    ...
    it('should initialize empty hashHistory if not provided', () => {
      const props = {
        ...
        currentOwner: 'owner-public-key',
        hashPreviousOwner: 'previous-owner-hash',
        hashHistory: undefined as unknown as string[]
      };
  ...
  describe('create', () => {
    it('should create a new Telomeer with initial owner', () => {
      ...
      const telomeer = Telomeer.create(tokenId, initialOwner);
      
      expect(telomeer.tokenId).toBe(tokenId);
      expect(telomeer.currentOwner).toBe(initialOwner);
      expect(telomeer.hashPreviousOwner).toBe(Telomeer.hashOwner('GENESIS'));
      expect(telomeer.hashHistory).toEqual([]);
    });
  });
  ...
  describe('hashOwner', () => {
    it('should create a SHA-256 hash of the owner public key', () => {
      ...
      const hash = Telomeer.hashOwner(ownerPublicKey);
    ...
    it('should produce different hashes for different keys', () => {
      const hash1 = Telomeer.hashOwner('owner1');
      const hash2 = Telomeer.hashOwner('owner2');
    ...
    it('should produce the same hash for the same key', () => {
      ...
      const hash1 = Telomeer.hashOwner(key);
      const hash2 = Telomeer.hashOwner(key);
  ...
  describe('transferOwnership', () => {
    ...
    beforeEach(() => {
      telomeer = Telomeer.create(tokenId, initialOwner);
    });
    
    it('should transfer ownership to a new owner', () => {
      const newOwner = 'new-owner';
      const transactionId = 'transaction-id';
      const previousOwnerHash = Telomeer.hashOwner(initialOwner);
    ...
    it('should prune history when it exceeds maximum size', () => {
      ...
      const telomeerWithLargeHistory = new Telomeer({
        tokenId,
  ...
  describe('verifyPreviousOwnership', () => {
    ...
    beforeEach(() => {
      telomeer = Telomeer.create(tokenId, owner1);
  ...
  describe('generateOwnershipProof', () => {
    it('should generate a proof with the correct structure', () => {
      const telomeer = Telomeer.create(tokenId, 'owner');
  ...
  describe('validate', () => {
    ...
    it('should return false if tokenId is invalid', () => {
      ...
      const telomeer = Telomeer.create(invalidTokenId, 'owner');
    ...
    it('should return false if hashPreviousOwner is invalid', () => {
      const telomeer = Telomeer.create(tokenId, 'owner');
      (telomeer as any)._hashPreviousOwner = '';
    ...
    it('should return false if hashHistory is not an array', () => {
      const telomeer = Telomeer.create(tokenId, 'owner');
      (telomeer as any)._hashHistory = 'not-an-array';
  ...
  describe('toJSON / fromJSON', () => {
    it('should correctly serialize and deserialize a Telomeer', () => {
      ...
      const original = Telomeer.create(tokenId, owner1);
      original.transferOwnership(owner2, 'tx1');
      
      const json = original.toJSON();
      const deserialized = Telomeer.fromJSON(json);
