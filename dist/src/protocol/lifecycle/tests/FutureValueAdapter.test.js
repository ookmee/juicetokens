...
describe('FutureValueAdapter', () => {
    let adapter;
    beforeEach(() => {
        adapter = new FutureValueAdapter_1.FutureValueAdapter();
    });
                ...
                verificationRequirements: [
                    {
                        ...
                        requirementType: FutureValueAdapter_1.RequirementType.EVIDENCE,
    ...
    describe('updateFulfillment', () => {
        it('should update fulfillment tracking for a promise', async () => {
            const result = await adapter.updateFulfillment('promise-123', 'req-1', 50, 'Halfway done with the widgets', ['photo-1.jpg']);
            expect(result.success).toBe(true);
            expect(result.data).toBeDefined();
            if (result.success && result.data) {
                ...
                expect(result.data.state).toBe(FutureValueAdapter_1.FulfillmentState.IN_PROGRESS);
            }
        });
        it('should mark a promise as fulfilled when completion is 100%', async () => {
            const result = await adapter.updateFulfillment('promise-123', 'req-1', 100, 'All widgets completed', ['photo-1.jpg', 'photo-2.jpg']);
            expect(result.success).toBe(true);
            expect(result.data).toBeDefined();
            if (result.success && result.data) {
                expect(result.data.promiseId).toBe('promise-123');
                expect(result.data.state).toBe(FutureValueAdapter_1.FulfillmentState.FULFILLED);
            }
        });
    });
...
describe('EscrowAdapter', () => {
    let adapter;
    beforeEach(() => {
        adapter = new FutureValueAdapter_1.EscrowAdapter();
    });
    describe('createEscrow', () => {
        it('should create an escrow with valid parameters', async () => {
            const result = await adapter.createEscrow('depositor-123', 'recipient-456', ['token-1', 'token-2'], [
                {
                    conditionId: 'cond-1',
                    description: 'Time-based release',
                    conditionType: FutureValueAdapter_1.ConditionType.TIME_BASED,
            ...
            if (result.success && result.data) {
                ...
                expect(result.data.status).toBe(FutureValueAdapter_1.EscrowStatus.ACTIVE);
            }
        });
        it('should reject an escrow with no tokens', async () => {
            const result = await adapter.createEscrow('depositor-123', 'recipient-456', [], [
                {
                    conditionId: 'cond-1',
                    description: 'Time-based release',
                    conditionType: FutureValueAdapter_1.ConditionType.TIME_BASED,
...
describe('CommunalPoolAdapter', () => {
    let adapter;
    beforeEach(() => {
        adapter = new FutureValueAdapter_1.CommunalPoolAdapter();
    });
    describe('createGroupCommitment', () => {
        it('should create a group commitment with valid parameters', async () => {
            ...
            const result = await adapter.createGroupCommitment('group-123', 'Community garden project', participants, contributions, Date.now() + 30 * 24 * 60 * 60 * 1000 // 30 days from now
            );
            expect(result.success).toBe(true);
            expect(result.data).toBeDefined();
            if (result.success && result.data) {
        ...
        it('should reject a commitment with missing participants', async () => {
            const result = await adapter.createGroupCommitment('group-123', 'Community garden project', [], {}, Date.now() + 30 * 24 * 60 * 60 * 1000);
            expect(result.success).toBe(false);
            expect(result.errors).toBeDefined();
            if (!result.success && result.errors) {
    ...
    describe('createRiskDistribution', () => {
        it('should create a risk distribution with valid parameters', async () => {
            ...
            const result = await adapter.createRiskDistribution('commitment-123', FutureValueAdapter_1.RiskStrategy.CUSTOM, customWeights);
            expect(result.success).toBe(true);
            expect(result.data).toBeDefined();
            if (result.success && result.data) {
                expect(result.data.distributionId).toBeDefined();
                expect(result.data.parentCommitmentId).toBe('commitment-123');
                expect(result.data.strategy).toBe(FutureValueAdapter_1.RiskStrategy.CUSTOM);
                expect(result.data.riskWeights).toEqual(customWeights);
            }
        });
        it('should reject a custom distribution with no weights', async () => {
            const result = await adapter.createRiskDistribution('commitment-123', FutureValueAdapter_1.RiskStrategy.CUSTOM);
            expect(result.success).toBe(false);
            expect(result.errors).toBeDefined();
            if (!result.success && result.errors) {
    ...
    describe('createCollectiveFulfillment', () => {
        it('should create a collective fulfillment with valid parameters', async () => {
            ...
            const result = await adapter.createCollectiveFulfillment('commitment-123', verifiers, 2 // 2-of-3 verification
            );
            expect(result.success).toBe(true);
            expect(result.data).toBeDefined();
            if (result.success && result.data) {
        ...
        it('should reject a fulfillment with invalid threshold', async () => {
            const result = await adapter.createCollectiveFulfillment('commitment-123', ['verifier-1', 'verifier-2'], 3 // Threshold > number of verifiers
            );
            expect(result.success).toBe(false);
            expect(result.errors).toBeDefined();
            if (!result.success && result.errors) {
    ...
    describe('recordVerificationVote', () => {
        it('should record a verification vote', async () => {
            const result = await adapter.recordVerificationVote('fulfillment-123', 'verifier-1', true, 'Looks good to me');
            expect(result.success).toBe(true);
            expect(result.data).toBeDefined();
            if (result.success && result.data) {
...
//# sourceMappingURL=FutureValueAdapter.test.js.map