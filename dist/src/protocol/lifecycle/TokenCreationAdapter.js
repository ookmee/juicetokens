...
exports.TokenCreationAdapter = void 0;
 ...
 */
class TokenCreationAdapter {
    constructor() {
        this.MIN_GENERATORS = 3;
        this.MAX_GENERATORS = 12;
        this.MIN_POTENCY_LEVEL = 1;
        this.MAX_POTENCY_LEVEL = 100;
        this.VALID_DENOMINATIONS = [1, 2, 5, 10, 20, 50, 100, 200, 500];
    }
    /**
    ...
    async generateEggs(params) {
        ...
        for (let i = 0; i < params.count; i++) {
            const eggId = `${params.batchReference}-${(0, uuid_1.v4)()}`;
            // Create default hatching condition
            const hatchingCondition = {
                eggId,
                conditionType: types_1.HatchingConditionType.ATTESTATION_THRESHOLD,
                conditionParameters: new Uint8Array(Buffer.from(JSON.stringify({ threshold: 3 }))),
     ...
     */
    async createGenesisPool(params) {
     ...
     */
    async triggerFertilization(dormantEgg, trigger) {
        const validationErrors = [];
        ...
        if (dormantEgg.dormancyExpiresMs < Date.now()) {
            validationErrors.push({
                ...
                code: types_1.ValidationErrorCode.EXPIRED
            });
            return {
                success: false,
                errors: validationErrors
        ...
        const spermComponent = {
            componentId: `sperm-${(0, uuid_1.v4)()}`,
     ...
     */
    updateMaturationPath(currentPath, newStage) {
     ...
     */
    createTokenDistribution(params) {
        const validationErrors = [];
        // Validate batch ID
        if (!params.batchId) {
            validationErrors.push({
        ...
        if (params.strategy === types_1.DistributionStrategy.WEIGHTED &&
    ...
    validateEggGeneration(params) {
        ...
        // Check denomination
        if (!this.VALID_DENOMINATIONS.includes(params.denomination)) {
            errors.push({
                field: 'denomination',
     ...
     */
    generateDormancySeal(eggId, entropyCommitment) {
        // Use SHA256 hash stacking instead of HMAC
        return crypto_1.HashUtil.createCommitment(eggId, entropyCommitment);
    }
    /**
     ...
     */
    calculatePotencyLevel(activityProof) {
        // Simple hash-based calculation for now
        const hash = crypto_1.HashUtil.sha256(activityProof);
        const firstByte = hash[0];
        // Map first byte (0-255) to potency level (1-100)
        return Math.max(this.MIN_POTENCY_LEVEL, Math.min(this.MAX_POTENCY_LEVEL, Math.floor(firstByte / 255 * this.MAX_POTENCY_LEVEL) + 1));
    }
    /**
     ...
     */
    generateCompatibilityProof(eggComponent, spermComponent) {
        // Use SHA256 hash stacking instead of HMAC
        return crypto_1.HashUtil.createCommitment(eggComponent.componentId, spermComponent.componentId, eggComponent.issuerId, spermComponent.generatorId);
    }
}
exports.TokenCreationAdapter = TokenCreationAdapter;
//# sourceMappingURL=TokenCreationAdapter.js.map