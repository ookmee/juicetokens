...
const TokenId_1 = require("../../../../packages/token/src/models/TokenId");
const Token_1 = require("../../../../packages/token/src/models/Token");
const types_1 = require("../types");
describe('Transaction Flow', () => {
    // Mock token creation helper
    const createMockToken = (id, denomination, issuer = 'test-issuer') => {
        const tokenId = new TokenId_1.TokenId({
            id,
    ...
    const createMockPublicKey = (name) => {
    ...
    test('Complete transaction flow (happy path)', () => {
        ...
        const senderManager = new TransactionManager_1.TransactionManager();
        const receiverManager = new TransactionManager_1.TransactionManager();
        ...
        }, senderTokens, receiverPublicKey);
        // Validate initiation
        ...
        const response = receiverManager.respondToTransaction(initiation, receiverTokens, true // Accepting the transaction
        );
        // Validate response
        ...
        if (response.senderExoPak && response.receiverExoPak) {
            ...
            const totalValue = response.senderExoPak.tokens.reduce((sum, token) => sum + token.denomination, 0);
        ...
        if (confirmation) {
            ...
            expect(finalizedTransaction.state).toBe(types_1.TransactionState.COMMITTED);
            ...
            expect(retrievedTransaction?.state).toBe(types_1.TransactionState.COMMITTED);
        }
    });
    test('Transaction rejection flow', () => {
        ...
        const response = receiverManager.respondToTransaction(initiation, receiverTokens, false, // Reject the transaction
        ...
        const retrievedTransaction = senderManager.getTransaction(initiation.transactionId);
        expect(retrievedTransaction).toBeDefined();
        expect(retrievedTransaction?.state).toBe(types_1.TransactionState.ABORTED);
    });
    test('Transaction with DenominationVectorClock optimization', () => {
        ...
        // Step 2: Receiver responds to the transaction
        const response = receiverManager.respondToTransaction(initiation, receiverTokens, true);
        expect(response.accepted).toBe(true);
...
//# sourceMappingURL=TransactionFlow.test.js.map