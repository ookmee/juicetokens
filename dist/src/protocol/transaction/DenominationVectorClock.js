...
exports.DenominationVectorClock = exports.DenominationStatus = void 0;
const Token_1 = require("../../../packages/token/src/models/Token");
/**
...
(function (DenominationStatus) {
    DenominationStatus["LACK"] = "00";
    DenominationStatus["SLIGHTLY_WANTING"] = "01";
    DenominationStatus["GOOD"] = "10";
    DenominationStatus["ABUNDANCE"] = "11";
})(DenominationStatus || (exports.DenominationStatus = DenominationStatus = {}));
/**
...
class DenominationVectorClock {
    ...
    constructor(currentDistribution) {
        ...
        if (currentDistribution) {
            ...
            Token_1.VALID_DENOMINATIONS.forEach(denom => {
     ...
     * @param currentDistribution Current distribution of tokens by denomination
     */
    calculateStatusCodes(currentDistribution) {
        Token_1.VALID_DENOMINATIONS.forEach(denom => {
     ...
     */
    updateStatus(denomination, status) {
        if (!Token_1.VALID_DENOMINATIONS.includes(denomination)) {
     ...
     */
    optimizeTokenSelection(availableTokens, targetAmount, receiverVectorClock) {
        // Count tokens by denomination
        const tokensByDenom = new Map();
        ...
        if (receiverVectorClock) {
            ...
            const sortedDenomsByReceiverNeed = [...Token_1.VALID_DENOMINATIONS].sort((a, b) => {
            ...
            for (const denom of sortedDenomsByReceiverNeed) {
                ...
                if (receiverStatus === DenominationStatus.LACK) {
                    ...
                    tokensToGive = Math.min(Math.floor(remainingAmount / denom), // Max based on remaining amount
                    ...
                    tokensToGive = Math.min(Math.floor(remainingAmount / denom), tokensOfDenom.length, senderStatus === DenominationStatus.ABUNDANCE ?
                    ...
                    if (senderStatus === DenominationStatus.ABUNDANCE) {
                        tokensToGive = Math.min(Math.floor(remainingAmount / denom), tokensOfDenom.length, Math.ceil(tokensOfDenom.length / 4) // Give up to a quarter
                        );
                    }
                }
        ...
        if (remainingAmount > 0) {
            ...
            const sortedDenoms = [...Token_1.VALID_DENOMINATIONS].sort((a, b) => b - a);
            for (const denom of sortedDenoms) {
                if (denom > remainingAmount)
                    continue;
                const tokensOfDenom = tokensByDenom.get(denom) || [];
                const tokensNeeded = Math.min(Math.floor(remainingAmount / denom), tokensOfDenom.length);
                if (tokensNeeded > 0) {
        ...
        if (remainingAmount > 0) {
            for (const denom of Token_1.VALID_DENOMINATIONS) {
                const tokensOfDenom = tokensByDenom.get(denom) || [];
     ...
     */
    toString() {
        const output = ['DenominationVectorClock:'];
        Token_1.VALID_DENOMINATIONS.forEach(denom => {
...
exports.DenominationVectorClock = DenominationVectorClock;
//# sourceMappingURL=DenominationVectorClock.js.map