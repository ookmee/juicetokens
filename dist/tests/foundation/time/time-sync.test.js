...
describe('Time Synchronization', () => {
    ...
    test('System time source should be available', async () => {
        const systemSources = timeSourceManager.getTimeSourcesByType(foundation_1.TimeSourceType.SYSTEM);
        expect(systemSources.length).toBe(1);
        const systemSource = systemSources[0];
        expect(systemSource).toBeInstanceOf(foundation_1.SystemTimeSource);
    ...
    test('Should add NTP source with mock mode', async () => {
        ...
        const ntpSource = new foundation_1.NTPTimeSource('ntp-test', mockLogger, {
            mockMode: true,
        ...
        const ntpSources = timeSourceManager.getTimeSourcesByType(foundation_1.TimeSourceType.NTP);
    ...
    test('Should use highest confidence source for time', async () => {
        ...
        ntpSource.updateConfidenceScore(95);
        // Add to manager
        timeSourceManager.addTimeSource(ntpSource);
    ...
    test('Should calculate consensus time from multiple sources', async () => {
        ...
        const ntpSource1 = new foundation_1.NTPTimeSource('ntp-test-1', mockLogger, {
            mockMode: true,
            mockTimeOffsetMs: 500, // 0.5 seconds ahead
            mockAvailable: true
        });
        const ntpSource2 = new foundation_1.NTPTimeSource('ntp-test-2', mockLogger, {
            mockMode: true,
...
//# sourceMappingURL=time-sync.test.js.map