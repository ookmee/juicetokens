...
const trust_1 = require("../../../src/protocol/trust");
describe('Trust and Attestation Layer', () => {
    ...
    beforeEach(() => {
        // Initialize services
        attestationService = new trust_1.AttestationService();
        dhtAdapter = new trust_1.DHTAdapter();
        reputationService = new trust_1.ReputationService(attestationService);
        identityService = new trust_1.IdentityService();
    });
    describe('Attestation Management', () => {
        test('creates and verifies a system attestation', () => {
            // Create a system attestation
            const systemAttestation = attestationService.createSystemAttestation('user123', 'system.verification', new Uint8Array([1, 2, 3, 4]), // Example evidence
            90, // Confidence score
            'verification-system', {
        ...
        test('creates and verifies a peer attestation', () => {
            ...
            const peerAttestation = attestationService.createPeerAttestation('user456', 'user789', 'peer.recommendation', 'I recommend this user for their reliability', 85, // Strength
            privateKey, {
        ...
        test('creates and verifies a community attestation', () => {
            ...
            const communityAttestation = attestationService.createCommunityAttestation('user123', 'community456', 'community.validation', 'This user has been validated by the community', 7, // Approval count
            5, // Threshold
        ...
        test('stores and retrieves attestations in the DHT', () => {
            ...
            const systemAttestation = attestationService.createSystemAttestation('user123', 'system.verification', new Uint8Array([1, 2, 3, 4]), 90, 'verification-system');
        ...
        test('queries attestations by subject ID', () => {
            ...
            attestationService.createSystemAttestation(subject, 'system.verification', new Uint8Array([1, 2, 3, 4]), 90, 'verification-system');
            attestationService.createPeerAttestation(subject, 'attester1', 'peer.recommendation', 'Recommendation', 85, new Uint8Array([1, 2, 3, 4, 5]));
            attestationService.createPeerAttestation(subject, 'attester2', 'peer.endorsement', 'Endorsement', 90, new Uint8Array([1, 2, 3, 4, 5]));
            // Query by subject ID
    ...
    describe('Reputation Management', () => {
        test('creates and updates a reputation profile', () => {
            ...
            const metric = reputationService.createReputationMetric(userId, 'technical-skill', 'Technical skill level', 0.8);
            // Verify custom metric was created
            expect(metric).toBeDefined();
            expect(metric.name).toBe('technical-skill');
            expect(metric.value).toBeCloseTo(0.8);
            // Update the metric
            const updated = reputationService.updateReputationMetric(userId, metric.metricId, 0.9);
        ...
        test('calculates reputation based on attestations', () => {
            ...
            attestationService.createPeerAttestation(userId, 'attester1', 'transaction', 'successful on-time transaction', 90, new Uint8Array([1, 2, 3, 4]));
            attestationService.createPeerAttestation(userId, 'attester2', 'transaction', 'successful transaction but delayed', 80, new Uint8Array([1, 2, 3, 4]));
            attestationService.createPeerAttestation(userId, 'attester3', 'contribution', 'attestation contribution to network', 85, new Uint8Array([1, 2, 3, 4]));
    ...
    describe('Identity Management', () => {
        test('creates and updates an identity', () => {
            // Create an identity
            const identity = identityService.createIdentity('John Doe', 'key123', {
            ...
            // Update an attribute
            const updated = identityService.setIdentityAttribute(identity.identityId, 'location', 'San Francisco', true, // Public
        ...
        test('creates identity links', () => {
            ...
            // Create a trust link
            const link = identityService.createIdentityLink(identity1.identityId, identity2.identityId, trust_1.LinkType.TRUSTS, 90, 'Trusted colleague', new Uint8Array([1, 2, 3, 4]), // Simulated signature
            true // Bidirectional
            );
            expect(link).toBeDefined();
            // Verify the link exists in both directions
            const hasLink1to2 = identityService.hasLink(identity1.identityId, identity2.identityId, trust_1.LinkType.TRUSTS);
            const hasLink2to1 = identityService.hasLink(identity2.identityId, identity1.identityId, trust_1.LinkType.TRUSTS);
        ...
        test('creates a pseudonym with privacy', () => {
            ...
            // Create a pseudonym
            const pseudonym = identityService.createPseudonym(realIdentity.identityId, 'Anon123', 'key2', new Uint8Array([1, 2, 3, 4]) // Simulated signature
            );
            expect(pseudonym).toBeDefined();
            expect(pseudonym?.displayName).toBe('Anon123');
            // Real identity should have a link to the pseudonym
            const hasLink = identityService.hasLink(realIdentity.identityId, pseudonym.identityId);
            expect(hasLink).toBe(true);
            // But the pseudonym should not have a link back (for privacy)
            const hasReverseLink = identityService.hasLink(pseudonym.identityId, realIdentity.identityId);
        ...
        test('manages selective disclosure of attributes', () => {
            ...
            const identity2 = identityService.createIdentity('Bob', 'key2');
            // Make email public
            identityService.setIdentityAttribute(identity1.identityId, 'email', 'alice@example.com', true, // Public
            new Uint8Array([1, 2, 3, 4]));
            // Create a trust link for selective disclosure
            identityService.createIdentityLink(identity1.identityId, identity2.identityId, trust_1.LinkType.TRUSTS, 80, 'Trusted friend', new Uint8Array([1, 2, 3, 4]));
            // Request disclosure of attributes
            const disclosure = identityService.createSelectiveDisclosure(identity1.identityId, ['email', 'phone', 'ssn'], identity2.identityId, new Uint8Array([1, 2, 3, 4]));
    ...
    describe('Integration Tests', () => {
        test('attestations update reputation and link to identity', () => {
            ...
            const attestation = attestationService.createPeerAttestation(identity1.identityId, // Alice is the subject
            identity2.identityId, // Bob is the attester
            'peer.endorsement', 'Alice is highly skilled in programming', 95, new Uint8Array([1, 2, 3, 4]) // Simulated private key for Bob
            );
            // Link the attestation to Alice's identity attribute
            identityService.addAttributeAttestation(identity1.identityId, 'programming-skill', attestation.attestationId, trust_1.AttributeVerificationStatus.PEER_VERIFIED);
            // Update Alice's reputation with the attestation
            reputationService.createReputationMetric(identity1.identityId, 'programming-skill', 'Programming skill level', 0.0 // Start at 0
            );
            ...
            if (metric) {
                reputationService.updateReputationMetric(identity1.identityId, metric.metricId, attestation.strength / 100, // Convert 0-100 to 0-1
            ...
            expect(progSkill?.verificationStatus).toBe(trust_1.AttributeVerificationStatus.PEER_VERIFIED);
        });
    });
});
//# sourceMappingURL=trust-layer.test.js.map