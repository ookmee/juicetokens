...
describe('Time Synchronization', () => {
    ...
    beforeEach(() => {
        ...
        timeSourceManager = new src_1.TimeSourceManager(mockLogger, {
            includeSystemTimeSource: true,
            systemTimeSourceId: 'system-time-test'
        });
        // Create a consensus manager
        consensusManager = new src_1.TimeConsensusManager(mockLogger);
    });
    afterEach(() => {
        // Clean up
    ...
    test('System time source should be available', async () => {
        const systemSources = timeSourceManager.getTimeSourcesByType(src_1.TimeSourceType.SYSTEM);
        expect(systemSources.length).toBe(1);
        const systemSource = systemSources[0];
        expect(systemSource).toBeInstanceOf(src_1.SystemTimeSource);
    ...
    test('Should add NTP source with mock mode', async () => {
        ...
        const ntpSource = new src_1.NTPTimeSource('ntp-test', mockLogger, {
            mockMode: true,
        ...
        const ntpSources = timeSourceManager.getTimeSourcesByType(src_1.TimeSourceType.NTP);
    ...
    test('Should calculate consensus time from multiple sources', async () => {
        ...
        const ntpSource1 = new src_1.NTPTimeSource('ntp-test-1', mockLogger, {
            mockMode: true,
            mockTimeOffsetMs: 500, // 0.5 seconds ahead
            mockAvailable: true
        });
        const ntpSource2 = new src_1.NTPTimeSource('ntp-test-2', mockLogger, {
            mockMode: true,
